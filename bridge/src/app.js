...
function stripMarkdownH3Sections(markdownText, headingPrefixes) {
  ...
  const shouldStripHeading = (line) =>
    prefixes.some((p) => String(line || "").startsWith(p));
...
function sanitizeCodexMarkdown(markdownText, options = {}) {
  ...
  if (!includeToolOutputs) {
    out = stripMarkdownH3Sections(out, [
      "### 工具输出",
      "### Tool output",
      "### Tool outputs",
    ]);
...
function createApp({ bridgeDb, bodyLimit }) {
  if (!bridgeDb) {
    throw new Error("createApp requires bridgeDb");
  }

  const app = express();
  app.use(express.json({ limit: bodyLimit || "25mb" }));
  ...
  app.post("/bridge/v1/projects/:project_id/sync/open-notebook", (req, res) => {
    ...
    Promise.resolve(notebookIdPromise)
      .then(async (notebookId) => {
        const projectOut = { id: projectRow.id, name: projectRow.name, cwd: projectRow.cwd };
        const sessionOut = { id: sessionRow.id, name: sessionRow.name };
